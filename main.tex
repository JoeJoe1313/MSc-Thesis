...
\noindent In summary, PSMF and rPSMF outperformed MLE-SMF and TMF for imputing missing values in 12-lead ECG time series data, achieving lower imputation errors, especially at rank $3$. Unexpectedly, higher rank led to worse performance for PSMF and rPSMF. rPSMF provided the best uncertainty quantification, but its coverage decreased at higher rank. Visual reconstructions showed that PSMF and rPSMF captured signal dynamics well at rank $3$, but had some issues at rank $10$. These results are promising, but further investigation is needed to understand the rank effects and to explore imputation in abnormal ECG rhythms.
...
\begin{equation}
    xk = fθ (xk 1) = θ1 sin(2πθ2k+θ3xk 1)+θ4 cos(2πθ5k+θ6xk 1
\end{equation}
We also use $r = 3$, run iterative PSMF with $100$ iterations, and withhold $10\%$ of the data for testing.
