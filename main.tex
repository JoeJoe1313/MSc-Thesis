...
\begin{itemize}
    \item We show three novel applications of PSMF to complex \textbf{12-lead ECG data}.
    \item \textbf{Application 1:} We apply both PSMF and rPSMF for imputing missing data in the ECG signals, and compare the results with other sequential probabilistic MF models.
...
\begin{align}
    -\log \Tilde{p}_{\theta}(y_k | y_{1:k-1}) &\overset{c}{=} \frac{d}{2} \log \left(\Vert f_{\theta}(\mu_{k-1}) \Vert_{V_{k-1}}^2 + \eta_k\right) + \nonumber \\
    &\quad + \left(\frac{\lambda_{k-1} + d}{2}\right) \log \left(1 + \frac{\Vert y_k - C_{k-1} f_{\theta}(\mu_{k-1}) \Vert^2}{\lambda_{k-1}\left(\Vert f_{\theta}(\mu_{k-1}) \Vert^2_{V_{k-1}} + \eta_k\right)}\right),
\end{align}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.35\linewidth]{images/vs.jpg}
