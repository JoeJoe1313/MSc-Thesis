...
\noindent  We can evaluate the effectiveness of our uncertainty quantification by calculating the percentage of missing values that fall within a two-standard-deviation ($2\sigma$) interval of the approximate posterior distribution. 

The results, presented in Table 3, demonstrate that our approach enhances the uncertainty quantification compared to the MLE-SMF method (TMF does not provide a posterior distribution). It's worth noting that other methods don't provide a posterior distribution for comparison.

Interestingly, rPSMF achieves a higher coverage percentage than PSMF on three datasets, which can be attributed to its sequential updating of noise covariances.
