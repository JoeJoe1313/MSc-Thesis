...
\noindent If we let $\Delta^2_{1,k} = (y_k - C_{k-1}\bar{\mu}_k)^T (C_{k-1} \bar{P}_k C_{k-1}^T + \bar{R}_k)^{-1} (y_k - C_{k-1}\bar{\mu}_k)$ and $\omega_k = (\lambda_{k-1} + \Delta^2_{1,k}) / (\lambda_{k-1} + d)$, then (\cite{akyildiz2021probabilistic}) $s_0 = s$, $s_k = \omega_k^{-1} s_{k-1}$, and for the noise covariances update we have $Q_k = \omega_k Q_{k-1}$, and $R_k = \omega_k R_{k-1}$. The updates for the mean for the coefficients and the dictionary are the same, but the updates for the coefficient covariance $P_k$ and the dictionary column-covariance $V_k$ undergo changes.
