...
We are going to denote the original data matrix as $Y$, and let $Y \in \mathbb{R}^{m \times n}$. Let $C \in \mathbb{R}^{m \times r}$ be the dictionary matrix, $X \in \mathbb{R}^{r \times n}$ be the coefficients matrix, and $r$ be the approximation rank. With $f_{\theta} $\newline

\noindent Throughout this thesis, we will denote the data matrix with $Y$, and the factors as $C$ and $X$. We denote the vectorised forms of the matrices with their respective lower case letters. We can formally define c = vec(C), where vec(·) is the vectorisation operation. The columns of C are stacked on each other, so if C ∈ Rm×r, then c ∈ Rmr×1. We can also define the inverse vectorisation operator C = vec−1(c). \newline
...
The PSMF model can be described with the following state-space equations:
...
\begin{align*}
p(\mathbf{C}) &= \mathcal{MN}(\mathbf{C}; \mathbf{C}_0, \mathbf{I}_d, \mathbf{V}_0) \\
p(\mathbf{x}_0) &= \mathcal{N}(\mathbf{x}_0; \boldsymbol{\mu}0, \mathbf{P}0) \\
p{\theta}(\mathbf{x}k | \mathbf{x}{k-1}) &= \mathcal{N}(\mathbf{x}k; f{\theta}(\mathbf{x}{k-1}), \mathbf{Q}_k) \\
p(\mathbf{y}_k | \mathbf{x}_k, \mathbf{C}) &= \mathcal{N}(\mathbf{y}_k; \mathbf{C}\mathbf{x}_k, \mathbf{R}_k)
\end{align*}
...
TODO: add figures showcasing the original data, missing parts and imputed parts
