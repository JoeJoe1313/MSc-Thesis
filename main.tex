...
\noindent For the updates ... full derivation can be found ... here we are going to state the results ... \newline
...
\begin{equation}
    \Tilde{p}(y_k | y_{1:k-1}, x_k) = \mathcal{N}(y_k; C_{k-1} x_k, \bar{R}_k),
\end{equation}
\begin{equation}
    \Tilde{p}(x_k | y_{1:k}) = \mathcal{x_k; \mu_k, P_k},
\end{equation}
...
The iterative version of the PSMF algorithm is given in Algorithm \ref{algo:1}. There also exists an online, recursive version of PSMF which is not given here but can be found in the original PSMF paper (\cite{akyildiz2021probabilistic}).
