...
Here, we aim to make a forecast with rPSMF. After numerous attempts with different hyperparameter settings we did not achieve better results than the ones from PSMF. Hence, we are going to define the rPSMF with the same parameters and subspace model as the ones set for PSMF in Section \ref{exp3:psmfmodel}. The only additional parameter is $\lambda_0 = 1.8$. We run iterative rPSMF with $400$ iterations, and withhold $20\%$ of the data for prediction. This allows us to compare the results for PSMF and rPSMF under very similar model settings.

\subsection{rPSMF Results}
